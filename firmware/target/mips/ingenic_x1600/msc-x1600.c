...
{
    unsigned rate = sdmmc_host_get_bus_freq(clock);
    if(rate == 0)
        panicf("%s: bad clock", __func__);
                          ...
                          bool io_abort)
{
    ...
    if(cmd->flags & SDMMC_RESP_BUSY) {
        ...
        /* Wait for busy release; data commands select their own completion. */
        iflag_done = jz_orm(MSC_IFLAG, PROG_DONE);
    }
    ...
    /* Keep completion and aborts out until DMA and the timeout are ready. */
    int irq = disable_irq_save();
...
static bool msc_cd_check(msc_drv* d)
{
    bool present = msc_card_detect(d);

    /* Abort on the same sample the poller reports, before the storage
     * thread processes the removal event. */
    if(!present)
        msc_abort_command(d);
...
static bool msc0_cd_check(void)
{
    return msc_cd_check(&msc_drivers[0]);
}

static bool msc1_cd_check(void)
{
    return msc_cd_check(&msc_drivers[1]);
}

static void msc_cd_interrupt(msc_drv* d)
{
    sdmmc_poll_event(&d->cd_poll);
...
static void msc_init_one(msc_drv* d, int msc)
{
    ...
    if(host_config.is_removable) {
        sdmmc_poll_init(&d->cd_poll, &d->host,
                        msc == 0 ? msc0_cd_check : msc1_cd_check);
        sdmmc_host_init_medium_present(&d->host, d->cd_poll.is_inserted);

        /* The host and poller must be ready before an IRQ can arrive. */
...
/* Use MPLL: 1400 MHz divided by two supports exact 25/50 MHz card clocks.
 * The vendor MSC clock-source selection has not been captured. */
#define MSC_CLOCK_SOURCE  X1600_CLK_MPLL
#define MSC_CLOCK_MPCS    BV_CPM_MSC1CDR_MPCS__MPLL
    {
        /* MSC1 pin assignment: vendor GPIO labels and PM Table 21-5. */
        .msc_nr = 1,
        ...
        /* PC25 has a pull-down, so drive the power switch explicitly. */
    ...
    /* MSC0 is connected to the unsupported Wi-Fi module. */
#else
...
{
    /* Use the slowest divider until msc_apply_bus_clock() sets the rate. */
...
unsigned msc_soc_set_clock(int msc, unsigned rate)
{
    ...
    /* Reject a stopped clock source before calculating the divider. */
