...
/* CLKGR bit 5 = MSC1, bit 4 = MSC0. Nothing may touch an MSC register before
 * msc_gate_clock(msc, false): on the X1600 a gated read hangs the bus. */
...
void msc_set_speed(msc_drv* d, int rate)
{
    ...
    /* freq1 is output by the CPM divider; freq2 is output by MSC_CLKRT */
    uint32_t freq1 = rate;
    uint32_t freq2 = rate;
    if(freq1 < MSC_SPEED_FAST)
        freq1 = MSC_SPEED_FAST;

    /* Handle the CPM divider */
    uint32_t in_freq = msc_soc_set_clock(d->msc_nr, freq1);

    /* Handle MSC_CLKRT.
     *
     * clk_calc_shift() returns one less than documented for non-power-of-two
     * ratios, so identification can run above the SD spec's 400 kHz ceiling.
     * FAST and HIGH are exact, so the overshoot is confined to identification,
     * and every shipping target already does this. */
...
void msc_async_start(msc_drv* d, msc_req* r)
{
    ...
    if(r->flags & MSC_RF_DATA) {
        ...
        /* DMASEL=0 selects the MSC's own SDMA rather than the PDMA, so the
         * PDMA request-type encodings do not apply to this transfer. */
...
static void msc_interrupt(msc_drv* d)
{
    ...
    /* Cannot normally fire -- an unconfigured controller never has its INTC
     * source unmasked -- but a NULL deref from an ISR is worse than a panic */
    if(UNLIKELY(d->config == NULL))
        panicf("msc: IRQ from an unconfigured controller");
...
/* These two names are the weak-alias slots in the SoC's irqvector[] for the
 * MSC0 and MSC1 INTC sources; defining them takes over the vector. */
void MSC0(void)
{
