...
\nopt{hibyr1native}{Installing Rockbox is generally a quick and easy procedure. However
...
\nopt{gigabeats,fiiom3k,shanlingq1,hibyr1native}{
\begin{description}
...
\opt{hibyr1native}{
  Rockbox Utility does not currently support the native port to the \playername{}.
  This manual does not yet provide an initial bootloader installation procedure.
  The firmware copying and update instructions below assume that the native
  Rockbox bootloader is already installed.
}
...
\item[The Rockbox bootloader.] \index{Bootloader}
  ...
  \dap{} how to load and start other components of Rockbox%
  \nopt{hibyr1native}{ and for providing the dual boot function}. This is the
  component of Rockbox that is installed to the flash memory of your \playerman.
  \opt{iaudiom3,iaudiom5,iaudiox5}{\note{Dual boot does not currently work on the \playertype.}}
  \opt{hibyr1native}{\note{The native Rockbox bootloader only boots Rockbox;
    dual boot is not supported. Installing it erases the original firmware's
    first-stage bootloader (SPL).}}}
...
\nopt{hibyr1native}{\subsection{Automated Installation}}
...
\nopt{hibyr1native}{\subsubsection{Choosing a Rockbox version}}
\opt{hibyr1native}{\subsection{Choosing a Rockbox version}}
...
\nopt{hibyr1native}{Please now go to \reference{ref:finish_install} to complete the installation procedure.}
...
\opt{hibyr1native}{
  These steps copy the Rockbox firmware to a FAT32-formatted microSD card;
  they do not install the native bootloader. Use a build for \fname{hibyr1native}.
}
...
\opt{hibyr1native}{
\item Power off your \dap{}, remove the microSD card and connect it to your
...
\item Take the \fname{.zip} file that you downloaded and use
 ...
 \opt{rgnano,hibyr1native}{the root of the microSD card of }%
 your \dap{}.
...
\opt{swcodec}{%
    ...
    \subsubsection{Installing the bootloader}
        ...
        \opt{hibyr1native}{
          Initial installation of the native bootloader is not yet documented
          in this manual. The installation procedures for other players do not
          apply to the \playername{}.
        }
    }
}
...
\opt{hibyr1native}{
  Safely eject / unmount the microSD card, insert it into your powered-off
  \dap{} and turn it on. This requires an already installed native bootloader.
}
...
allow Rockbox to speak the user interface to you. \nopt{hibyr1native}{Rockbox Utility can install
...
\opt{hibyr1native}{If a voice file is available for your native build, unzip it
...
on voice settings.
\nopt{hibyr1native}{Rockbox Utility can also aid in the production of talk files, which allow Rockbox
to speak file and folder names.}

\section{Running Rockbox}
\nopt{ipod,gigabeats,cowond2,rgnano,hibyr1native}{When
you turn the unit on, Rockbox should load.}
\opt{hibyr1native}{With the native bootloader installed and the prepared
microSD card inserted, Rockbox should load when you turn the \dap{} on.}
...
of your \dap{}\opt{hibyr1native}{'s microSD card} as in the manual installation stage. If your unzip
...
\nopt{gigabeatf,iaudiom3,iaudiom5,iaudiox5,mrobe100,gigabeats,fuzeplus,erosqnative,rgnano,hibyr1native}{
  \note{The Rockbox bootloader allows you to choose between Rockbox and
  the original firmware. (See \reference{ref:Dualboot} for more information.)}
}

\opt{hibyr1native}{
  Rockbox Utility cannot uninstall the native port. Removing the
  \fname{.rockbox} directory from the microSD card does not restore the original
  firmware's first-stage bootloader (SPL) and will leave Rockbox unable to boot.
  A procedure for removing the native bootloader and restoring the original
  firmware is not yet documented in this manual.
}

\nopt{hibyr1native}{\subsection{Automatic Uninstallation}}
\opt{gigabeats,erosqnative,rgnano}{\note{Rockbox can only be uninstalled manually from the \playername{} for now.}}

\nopt{erosqnative,rgnano,hibyr1native}{
...
\nopt{hibyr1native}{\subsection{Manual Uninstallation}}
...
\nopt{gigabeats,rgnano,hibyr1native}{
...
\opt{hibyr1native}{
  \item[Could not start Rockbox.]
  Check that the microSD card is formatted as FAT32 and contains the entire
  extracted build at its root, including \fname{.rockbox/rockbox.r1}.
  See \reference{sec:installing_firmware} for the file copying instructions.
}
